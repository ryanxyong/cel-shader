...
renderer.setClearColor(0x000000); // For easier viewing of shading result
	...
	if (child instanceof THREE.Mesh) { // if child is a Mesh
		if (child.material.id in mats) { // if material has already been processed
			...
			mats[child.material.id] = new THREE.ShaderMaterial({ // process texture through toon shader
				uniforms: {
					MaterialTexture: {value: child.material.map},
					LightPosition: {value: keyLight.position}
				},
				vertexShader: _VS,
				fragmentShader: _FS
			});
...
// const geometry = new THREE.TorusKnotGeometry(0.6, 1);
// //const material = new THREE.MeshBasicMaterial( { color: 0x00ff00 } );
// const cubeMat = new THREE.ShaderMaterial({vertexShader: _VS,
// 	fragmentShader: _FS});
